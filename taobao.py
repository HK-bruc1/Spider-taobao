...
class TaoBao:
    ...
    def crawl_good_buy_data(self):
        ...
        for page in range(1, 7):
            ...
            for good in goods:
                ...
                # 保存一单
                good_list.append(goods_time)
                good_list.append(goods_name)
                good_list.append(goods_money)
            ...
            time.sleep(2)
        # 保存所有数据
        self.save_data(goods_list, number)
        f.close()
    ...
    # 保存数据
    def save_data(self, save_list, num):
        print("获取数据中...")
        # 注意是左闭右开
        for i in range(0, num):  # 保存所有商品后一起存入
...
if __name__ == "__main__":
    ...
    all_list = []  # 保存所有文章的信息
    # 保存为csv格式
    f = open("taobao.csv", mode="w", encoding="utf-8", newline="")
    writer = csv.writer(f)
    taobao_username = "+++++++"  # 改成你的账号
    taobao_password = "+++++++"  # 改成你的密码
    a = TaoBao()
    a.login()  # 登录
